...
const int end_button = 34; //pin connected to button to end the entire game. 
...
void loop(void) {
  ...
  display_switch();
  clear_pot_button();
  end_game_sm(digitalRead(end_button));
}

void end_game_sm(int button_val){
  switch(end_button_state){
    case IDLE:
      if(button_val == 0)
        end_button_state = PRESS;
      break;
    case PRESS:
      if(button_val == 1)
        end_button_state = RELEASE;
      break;
    case RELEASE:
      end_game();
      break;
...
void end_game(){
  ...
  sprintf(body, "game=current&end=true");
...
//int pot_size;
char* pot_get_request(){ //gets current pot size
  sprintf(request_buffer, "GET http://608dev-2.net/sandbox/sc/team20/webserver/pot.py HTTP/1.1\r\n");
  strcat(request_buffer, "Host: 608dev-2.net\r\n");
  strcat(request_buffer, "\r\n");                                                        //new line from header to body
  Serial.println(request_buffer);
  do_http_request("608dev-2.net", request_buffer, response_buffer, OUT_BUFFER_SIZE, RESPONSE_TIMEOUT, true);
  Serial.println(response_buffer);  //viewable in Serial Terminal
  //pot_size = response_buffer;
...
uint8_t pot_state = 0;
void clear_pot_button(){
  if (pot_state == 0){
    if (digitalRead(pot_button) == 0){
      pot_state = 1;
    }
  }
  else if (pot_state == 1){
    if (digitalRead(pot_button) == 1){
      pot_state = 2;
    }
  }
  else if (pot_state == 2){
  ...
  switch(display_state){
    case DISPLAY_POT:
      ...
      if(strcmp(username, default_username)!=0 && strlen(username)!=0){
        ...
        tft.setCursor(0, 0, 1);
        tft.printf("%s is at %s", username, location);
        ledcWriteTone(AUDIO_PWM, 300);
      }
      break;
    case DISPLAY_GUEST:
      tapIn();
...
void potSize(){
  char printstr[100];
  char* str_pot= pot_get_request();
  char str_start[100] = "Pot Size:\n";
  matrix.setTextColor(matrix.color565(34, 110, 55));
  matrix.setTextSize(1);
  matrix.setCursor(0,0);
  strcat(str_start,str_pot);
  matrix.fillScreen(0);
  matrix.printf("%s",str_start);
  matrix.show();
}
